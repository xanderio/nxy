...
with subtest("copy store path to agent"):
    ...
    server.succeed(f"NXY_SERVER=http://server nxy agents download {agent_id} {test_store_path}")
    alpha.succeed(f"nix path-info {test_store_path}")

with subtest("activate configuration"):
    server.succeed(f"NXY_SERVER=http://server nxy agents activate {agent_id} {test_store_path}")
